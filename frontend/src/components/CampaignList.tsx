...
import { Loader2, Calendar, DollarSign, Users, Briefcase, ChevronRight, AlertCircle } from 'lucide-react';
...
// Định nghĩa lại Interface cho đúng với phản hồi mới từ Backend
interface Campaign {
  id: string;
  title: string;
  budget: number;
...
// Interface cho cấu trúc Phân trang
...
export default function CampaignList() {
  ...
  // Cập nhật useQuery để nhận đúng kiểu dữ liệu phân trang
  const { data: response, isLoading, error } = useQuery<PaginatedResponse>({
    queryKey: ['campaigns', page], // Thêm page vào key để trigger fetch lại khi đổi trang
    queryFn: async () => await api.get(`/campaigns?page=${page}&take=9`),
  });
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-destructive/20 bg-destructive/10 p-6 text-center text-destructive flex flex-col items-center">
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {campaigns.map((campaign) => (
          <Card key={campaign.id} className="group flex flex-col overflow-hidden hover:border-primary/50 cursor-pointer transition-all hover:shadow-md">
            <Link href={`/campaigns/${campaign.id}`} className="flex flex-col h-full">
              ...
              <CardContent className="pb-4 flex-1">
                 <div className="grid grid-cols-2 gap-3 mt-2">
                    <div className="space-y-1 p-2.5 rounded-lg bg-secondary/40 border border-border/50">
              ...
              <CardFooter className="border-t bg-muted/10 py-3">
                <div className="flex w-full items-center justify-between">
                  <div className="flex items-center text-xs text-muted-foreground font-medium">
                    ...
                    {campaign._count?.applications || 0} ứng viên
                  </div>
                  <Button variant="ghost" size="sm" className="h-8 text-xs hover:bg-primary/10 hover:text-primary gap-1 px-2">
